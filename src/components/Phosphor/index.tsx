...
interface AppState {
  ...
  adminAccessEnabled: boolean; // whether admin has granted access to restricted areas
}
...
class Phosphor extends Component<any, AppState> {
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      adminAccessEnabled: false, // default to no admin access
    };
  ...
  public componentDidMount(): void {
    ...
    // Initialize admin access listener
    const { initializeAdminAccess } = require("../../utils/adminAccess");
  ...
  private _buildScreen(src: any): Screen {
    ...
    const content = this._parseScreenContent(src.content, id).flat(); // flatten to one dimension
  ...
  private _parseScreenContent(content: any[], screenId: string): ScreenData[] {
    if (!content) {
      return [];
    }
    ...
    return parsed.map((element, index) =>
      this._generateScreenData(element, screenId, index),
    );
  }
  ...
  private _generateScreenData(
    ...
    // Create deterministic ID based on screen and position for consistent cross-client identification
    const id = `${screenId}_${index}`;
  ...
  private _renderStaticElement(element: any, key: number): ReactElement {
    ...
    if (element.type === ScreenDataType.Text) {
      // Use non-breaking space to ensure empty lines aren't collapsed
      // \u00A0 is the Unicode non-breaking space character
      const text = element.text.length ? element.text : "\u00A0";
      return (
        <Text
          key={key}
          className={className}
  ...
  private _handleLinkClick(target: string | any[], shiftKey: boolean): void {
    ...
    // Check if admin access is enabled - if so, treat it as if shift key is pressed
    const { isAdminAccessEnabled } = require("../../utils/adminAccess");
    const effectiveShiftKey = shiftKey || isAdminAccessEnabled();

    const linkTarget = (target as any[]).find(
      (element) => element.shiftKey === effectiveShiftKey,
    );
    if (linkTarget) {
